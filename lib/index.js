...
/**
 * Host half: exposes loopback endpoints so the client can render the pairing
 * QR without cross-origin navigation.
 *
 ...
 *
 * When remote-link is absent the probe reports `available: false` and the
 * client shows a hint instead of a dead image.
 */
export function apply(ctx) {
  const removeConfig = ctx.webServer.register({
    kind: 'exact',
  ...
  const removePng = ctx.webServer.register({
    kind: 'exact',
