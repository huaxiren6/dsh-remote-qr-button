  ...
  factory: () => {
    ...
    function apply(ctx) {
      ctx.effect(() => {
        ...
        button.id = BTN_ID
        button.title = '手机连接二维码'
        button.textContent = '📱'
        const overlay = document.createElement('div')
        overlay.id = 'dsh-remote-qr-overlay'
        overlay.innerHTML = `<div id="dsh-remote-qr-card"><div id="dsh-remote-qr-head"><span>手机连接 · 扫码配对</span><button id="dsh-remote-qr-close">×</button></div><iframe id="dsh-remote-qr-frame" title="手机连接二维码"></iframe><div id="dsh-remote-qr-missing" style="display:none"></div></div>`
        button.onclick = async () => {
          ...
          if (port === null) {
            ...
            missing.textContent = '未检测到 dsh-remote-link 网关。\n本插件是 dsh-remote-link 的配套 UI，请先安装 dsh-remote-link（github:BotonJ/dsh-remote-link）。'
          } else {
